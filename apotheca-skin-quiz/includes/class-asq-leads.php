...
/**
 * Lead capture log for Apotheca Skin Quiz.
 *
...
class ASQ_Leads {
    ...
    public static function add_lead( $finder_id, $email, $consent, $answers, $products ) {
         ...
         */
        do_action( 'asq_lead_recorded', $lead_id, array(
            'finder_id' => absint( $finder_id ),
            'email'     => sanitize_email( $email ),
    ...
    public function render_page() {
        ...
        if ( isset( $_GET['asq_action'], $_GET['lead'] ) && 'delete' === $_GET['asq_action'] ) {
            $lead_id = absint( $_GET['lead'] );
            check_admin_referer( 'asq_delete_lead_' . $lead_id );
        ...
        <div class="wrap">
            ...
            <form method="get" style="margin:12px 0;">
                ...
                <select name="finder">
                    <option value="0"><?php esc_html_e( 'All finders', 'apotheca-skin-quiz' ); ?></option>
                ...
                <span style="margin-left:8px;color:#646970;">
                    ...
                    printf( esc_html( _n( '%s submission', '%s submissions', $total, 'apotheca-skin-quiz' ) ), esc_html( number_format_i18n( $total ) ) );
                    ?>
                </span>
            </form>

            <?php if ( ! $leads ) : ?>
                <p><?php esc_html_e( 'No submissions yet. When a visitor completes a finder and enters their email, it will appear here.', 'apotheca-skin-quiz' ); ?></p>
            <?php else : ?>
                <table class="widefat striped">
                    <thead>
                        <tr>
                            ...
                            <th><?php esc_html_e( 'Finder', 'apotheca-skin-quiz' ); ?></th>
                            <th><?php esc_html_e( 'Answers', 'apotheca-skin-quiz' ); ?></th>
                            <th><?php esc_html_e( 'Recommended Products', 'apotheca-skin-quiz' ); ?></th>
                            <th></th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php foreach ( $leads as $lead ) : ?>
                            ...
                            <tr>
                                ...
                                <td><?php echo $lead->consent ? '<span style="color:#00a32a;">&#10003; ' . esc_html__( 'Yes', 'apotheca-skin-quiz' ) . '</span>' : esc_html__( 'No', 'apotheca-skin-quiz' ); ?></td>
                                    ...
                                    <?php if ( is_array( $answers_rows ) && $answers_rows ) : ?>
                                        <details>
                                            <summary style="cursor:pointer;">
                                                ...
                                                printf( esc_html( _n( '%s answer', '%s answers', count( $answers_rows ), 'apotheca-skin-quiz' ) ), esc_html( number_format_i18n( count( $answers_rows ) ) ) );
                                                ?>
                                            </summary>
                                ...
                                <td>
                                    <a href="<?php echo esc_url( $delete_url ); ?>" style="color:#b32d2e;" onclick="return confirm('<?php echo esc_js( __( 'Delete this submission?', 'apotheca-skin-quiz' ) ); ?>');"><?php esc_html_e( 'Delete', 'apotheca-skin-quiz' ); ?></a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
