...
class ASQ_Email {
    ...
    public function send_results_email() {
        ...
        if ( ! $consent ) {
            wp_send_json_error( array( 'message' => __( 'Please tick the box so we can send your results.', 'apotheca-skin-quiz' ) ) );
        }
    ...
    private function build_email_body( $finder_title, $reading, $args = array() ) {
        ...
        $sub_heading = ! empty( $email_styles['sub_heading'] ) ? $email_styles['sub_heading'] : __( 'Here are your results, in full.', 'apotheca-skin-quiz' );
