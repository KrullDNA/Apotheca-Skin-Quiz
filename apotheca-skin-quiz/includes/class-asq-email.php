...
/**
 * Handle email sending and session-based results URLs for Apotheca Skin Quiz.
 */
class ASQ_Email {
    ...
    public function send_results_email() {
        ...
        if ( ! is_email( $email ) || empty( $product_ids ) ) {
            wp_send_json_error( array( 'message' => __( 'Invalid email or no products found.', 'apotheca-skin-quiz' ) ) );
        }
        ...
        $email_styles = get_post_meta( $finder_id, '_asq_email_styles', true );
    ...
    private function record_lead( $finder_id, $email, $consent, $products_data, $product_ids, $day_night, $day_products, $night_products ) {
        ...
        $readable_answers = ASQ_Leads::resolve_answers( $finder_id, (array) $answers, (array) $followup_answers );
        ...
        return ASQ_Leads::add_lead( $finder_id, $email, $consent, $readable_answers, $products );
    }
    ...
    public function send_test_email() {
        ...
        if ( ! function_exists( 'wc_get_products' ) ) {
            wp_send_json_error( array( 'message' => __( 'WooCommerce is not active.', 'apotheca-skin-quiz' ) ) );
        }
        ...
        if ( empty( $sample_products ) ) {
            wp_send_json_error( array( 'message' => __( 'No published products found to preview with.', 'apotheca-skin-quiz' ) ) );
        }
    ...
    private function build_product_card( $product, $name, $category_label, $results_url = '' ) {
        ...
        $html .= esc_html__( 'SHOP NOW', 'apotheca-skin-quiz' );
        $html .= '</a>';
        $html .= '</td>';
        $html .= '</tr></table>';
    ...
    private function build_email_body( $finder_id, $finder_title, $product_ids, $products_data = array(), $results_url = '', $email_styles = array(), $day_night = false, $day_products = array(), $night_products = array() ) {
        ...
        $sub_heading = ! empty( $email_styles['sub_heading'] ) ? $email_styles['sub_heading'] : __( 'Based on your answers, here are your recommended products:', 'apotheca-skin-quiz' );
        $footer_text = ! empty( $email_styles['footer_text'] ) ? $email_styles['footer_text'] : __( 'This email was generated by Apotheca Skin Quiz.', 'apotheca-skin-quiz' );
        $header_image_url = '';
        ...
        if ( $results_url ) {
            ...
            $html .= esc_html__( 'SHOP THE LOOK', 'apotheca-skin-quiz' );
            $html .= '</a>';
            $html .= '</td></tr>';
        }
        ...
        if ( $day_night && ( ! empty( $day_products ) || ! empty( $night_products ) ) ) {
            ...
            if ( ! empty( $day_products ) ) {
                $html .= $render_lozenge( __( 'Day', 'apotheca-skin-quiz' ) );
                $html .= $render_group( $day_products );
            }
            // Night group.
            if ( ! empty( $night_products ) ) {
                $html .= $render_lozenge( __( 'Night', 'apotheca-skin-quiz' ) );
