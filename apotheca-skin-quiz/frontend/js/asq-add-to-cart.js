...
(function ($) {
    ...
    $(document).on('found_variation', function (e, variation) {
        ...
        asqLog('[PF ATC] found_variation fired, variation_id:', variation.variation_id,
            'target:', $target.get(0));
        ...
        if (!$item.length) {
            asqLog('[PF ATC] No listing item container found for found_variation');
            return;
        }

        var $btn = $item.find('.asq-atc-btn[data-asq-variable]');
        if (!$btn.length) {
            asqLog('[PF ATC] No asq-atc-btn found in listing item');
            return;
        }

        // Enable the button.
        asqLog('[PF ATC] Enabling button for product:', $btn.data('product_id'),
            'variation:', variation.variation_id);
        $btn.removeClass('asq-atc-btn--disabled');

        // Store variation data on the button for our click handler.
        $btn.data('variation_id', variation.variation_id);
        $btn.data('quantity', parseInt($btn.siblings('.asq-atc-qty').val(), 10) || 1);
    ...
    $(document).on('click', '.asq-atc-btn[data-asq-variable]', function (e) {
        e.preventDefault();
        ...
        if ($btn.hasClass('asq-atc-btn--disabled')) {
            return false;
        }
        ...
        asqLog('[PF ATC] Adding variable product to cart:', postData);
        ...
        // simple products).
        var ajaxUrl = (typeof asqAddToCart !== 'undefined' && asqAddToCart.ajax_url)
            ? asqAddToCart.ajax_url
            : (typeof asqFrontend !== 'undefined' && asqFrontend.ajax_url)
                ? asqFrontend.ajax_url
                : '/wp-admin/admin-ajax.php';

        var nonce = (typeof asqAddToCart !== 'undefined' && asqAddToCart.nonce)
            ? asqAddToCart.nonce
            : (typeof asqFrontend !== 'undefined' && asqFrontend.nonce)
                ? asqFrontend.nonce
                : '';

        postData.action = 'asq_add_to_cart_variable';
        ...
        $.post(ajaxUrl, postData, function (response) {
            ...
            asqLog('[PF ATC] Added to cart successfully, variation:', variationId);
    ...
    $(document).on('click', '.asq-atc-variation-btn', function (e) {
        e.preventDefault();
        e.stopImmediatePropagation();
        ...
        asqLog('[PF ATC] Beauty mode: adding variation to cart:', postData);
        ...
        $.post(ajaxUrl, postData, function (response) {
            ...
            if (!response || response.success === false) {
                ...
                $btn.text(asqFrontend.i18n.add_to_cart || 'Add to Cart');
                return;
            }
            ...
            asqLog('[PF ATC] Beauty mode: added variation', variationId, 'to cart');
        }).fail(function (jqXHR, textStatus) {
            $btn.removeClass('loading').text(asqFrontend.i18n.add_to_cart || 'Add to Cart');
    ...
    $(document).on('click', '.asq-shade-atc-btn', function (e) {
        ...
        asqLog('[PF ATC] Shade Cart: adding to cart', postData);
        ...
        $.post(ajaxUrl, postData, function (response) {
            ...
            asqLog('[PF ATC] Shade Cart: added variation', variationId);
        }).fail(function (jqXHR, textStatus) {
            $btn.removeClass('loading');
    ...
    $(document).on('click', '.asq-atc-btn.asq-atc-btn--disabled', function (e) {
        e.preventDefault();
        e.stopImmediatePropagation();
        return false;
    });
    ...
    $(document).on('change input', '.asq-atc-qty', function () {
        ...
        var $btn = $(this).siblings('.asq-atc-btn');
        $btn.attr('data-quantity', qty);
        $btn.data('quantity', qty);
    });

    // Shade Cart quantity sync.
    $(document).on('change input', '.asq-sc-qty', function () {
        var qty = parseInt($(this).val(), 10) || 1;
        var $btn = $(this).siblings('.asq-shade-atc-btn');
        $btn.attr('data-quantity', qty);
        $btn.data('quantity', qty);
    });
