 ...
 */
class ASQ_Category_Label_Widget extends Widget_Base {

    public function get_name() {
        return 'asq_category_label';
    }

    public function get_title() {
        return __( 'PF Category Label', 'apotheca-skin-quiz' );
    }
    ...
    protected function register_controls() {
        ...
        $this->add_control( 'label_base', array(
            'label'   => __( 'Base / Foundation Label', 'apotheca-skin-quiz' ),
            'type'    => Controls_Manager::TEXT,
            'default' => 'BASE',
        ) );

        $this->add_control( 'label_concealer', array(
            'label'   => __( 'Concealer Label', 'apotheca-skin-quiz' ),
            'type'    => Controls_Manager::TEXT,
            'default' => 'CONCEAL',
        ) );

        $this->add_control( 'label_lip', array(
            'label'   => __( 'Lip Label', 'apotheca-skin-quiz' ),
            'type'    => Controls_Manager::TEXT,
            'default' => 'LIP',
        ) );

        $this->add_control( 'label_cheek', array(
            'label'   => __( 'Cheek Label', 'apotheca-skin-quiz' ),
            'type'    => Controls_Manager::TEXT,
            'default' => 'CHEEK',
        ) );

        $this->add_control( 'label_lip_cheek', array(
            'label'   => __( 'Lip & Cheek Label', 'apotheca-skin-quiz' ),
            'type'    => Controls_Manager::TEXT,
            'default' => 'LIP + CHEEK',
        ) );

        $this->add_control( 'label_eye', array(
            'label'   => __( 'Eye Label', 'apotheca-skin-quiz' ),
            'type'    => Controls_Manager::TEXT,
            'default' => 'EYE',
        ) );

        $this->add_control( 'fallback_label', array(
            'label'       => __( 'Fallback Label', 'apotheca-skin-quiz' ),
            'type'        => Controls_Manager::TEXT,
            'default'     => '',
            'description' => __( 'Shown when no category is assigned. Leave empty to hide.', 'apotheca-skin-quiz' ),
        ) );

        $this->add_control( 'html_tag', array(
            'label'   => __( 'HTML Tag', 'apotheca-skin-quiz' ),
        ...
        $this->add_responsive_control( 'align', array(
            ...
            'default'   => 'left',
            'selectors' => array(
                '{{WRAPPER}} .asq-cl-wrap' => 'text-align: {{VALUE}};',
            ),
        ) );
        ...
        $this->add_responsive_control( 'label_padding', array(
            ...
            'selectors'  => array(
                '{{WRAPPER}} .asq-cl-label' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
            ),
        ) );
        ...
        $this->add_responsive_control( 'label_margin', array(
            ...
            'size_units' => array( 'px', 'em' ),
            'selectors'  => array(
                '{{WRAPPER}} .asq-cl-label' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
            ),
        ) );
        ...
        $this->add_control( 'label_border_radius', array(
            ...
            'size_units' => array( 'px', '%' ),
            'selectors'  => array(
                '{{WRAPPER}} .asq-cl-label' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
            ),
        ) );

        $this->add_control( 'label_orientation', array(
            'label'   => __( 'Orientation', 'apotheca-skin-quiz' ),
            'type'    => Controls_Manager::SELECT,
            'default' => 'horizontal',
            'options' => array(
                'horizontal' => __( 'Horizontal', 'apotheca-skin-quiz' ),
                'vertical'   => __( 'Vertical (top to bottom)', 'apotheca-skin-quiz' ),
                'vertical_r' => __( 'Vertical (bottom to top)', 'apotheca-skin-quiz' ),
            ),
        ) );
    ...
    protected function render() {
        ...
        if ( class_exists( 'ASQ_Ajax' ) ) {
            $category = ASQ_Ajax::get_matched_category( $product_id );
        }
        ...
        if ( 'vertical' === $orientation ) {
            $orient_class = ' asq-cl--vertical';
        } elseif ( 'vertical_r' === $orientation ) {
            $orient_class = ' asq-cl--vertical-r';
        }

        printf(
            '<div class="asq-cl-wrap%3$s"><%1$s class="asq-cl-label">%2$s</%1$s></div>',
            $tag,
            esc_html( $label ),
            $orient_class
        );
    ...
    protected function content_template() {
        ...
        var cls = 'asq-cl-wrap';
        if ( orient === 'vertical' )   cls += ' asq-cl--vertical';
        if ( orient === 'vertical_r' ) cls += ' asq-cl--vertical-r';
        #>
        <div class="{{{ cls }}}">
            <{{{ tag }}} class="asq-cl-label">{{{ label }}}</{{{ tag }}}>
        </div>
        <?php
    }
}
